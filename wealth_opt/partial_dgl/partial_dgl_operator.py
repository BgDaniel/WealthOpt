...
class PartialDifferentialOperator:
    """
    Base class for a partial differential operator in HJB equations.

    Represents a general operator L applied to a value function V(t, x),
    such as drift and diffusion terms from stochastic dynamics.

    Subclasses should implement the `apply` method.
    """

    def apply(self, t: float, x: np.ndarray, V: Callable[[float, np.ndarray], float]) -> float:
        """
        Apply the differential operator to the value function.
        ...
        t : float
            Current time.
        x : np.ndarray
            Current state (can be multidimensional).
        V : callable
            Function handle V(t, x) returning scalar value.
